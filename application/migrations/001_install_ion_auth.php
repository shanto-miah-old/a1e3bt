...
class Migration_Install_ion_auth extends CI_Migration {
	...
	public function up() {
		...
		// Drop table 'users_updated' if it exists
		$this->dbforge->drop_table($this->tables['users_updated'], TRUE);
		...
		$this->dbforge->add_key('id', TRUE);
		$this->dbforge->create_table($this->tables['users_updated']);

		// Dumping data for table 'users_updated'
		$data = [
			'user_id'	=>	1,
			'name'		=>	time(),
			'dob'		=> time()
		...
		$this->db->insert($this->tables['users_updated'], $data);
